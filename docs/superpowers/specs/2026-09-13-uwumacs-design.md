...
Conflicting non-user bindings at the same specificity are configuration errors. Validation names both owners and retains the last valid map set. No silent last-writer-wins behavior. User maps intentionally override defaults without error, except the reserved localleader suffix: put localleader overrides in the buffer-local user localleader map. P02 reconciles the live public base with retained foundation declarations before validating it together with contextual non-user sources and committing metadata alongside native maps. Matching native bindings keep declaration ownership and priority; new or changed base bindings use owner `public-base` at priority zero, and removed bindings contribute nothing. Base edits require `uwumacs-refresh` for reconciliation; native user override maps remain immediate. ADR-0005 defines the native fallback and reserved-prefix behavior. It builds all selected buffer candidates before committing any; failed Customize candidates preserve prior option values and maps. See ADR-0004 and ADR-0005 for the conservative terminal exclusions and concrete provider seam.
