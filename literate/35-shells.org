...
#+STARTUP: overview
#+PROPERTY: header-args:emacs-lisp :tangle ../lambda-library/lambda-user/uwumacs-shell.el :eval never :comments no :padline no :mkdirp yes
...
variables from a login shell. Windows inherits its environment correctly,
and [[file:30-platform.org][the platform chapter]] turns this off there.
