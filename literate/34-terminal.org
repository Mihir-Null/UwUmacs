...
#+STARTUP: overview
#+PROPERTY: header-args:emacs-lisp :tangle ../lisp/uwumacs-terminal.el :eval never :comments no :padline no :mkdirp yes
...
environment is untouched. [[file:30-platform.org][The platform chapter]] owns =uwumacs-msys2-root=.
...
exist yet. =consult-ghostel-history= completes against the *shell's own*
