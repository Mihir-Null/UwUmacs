...
* uwumacs-treesit
:PROPERTIES:
:header-args:emacs-lisp: :tangle ../lisp/uwumacs-treesit.el :eval never :comments no :padline no :mkdirp yes
:END:

Deployed file: [[file:../lisp/uwumacs-treesit.el][lisp/uwumacs-treesit.el]]. Edit the blocks here; generated Lisp is kept in Git for ordinary startup and review.
...
* uwumacs-languages
:PROPERTIES:
:header-args:emacs-lisp: :tangle ../lisp/uwumacs-languages.el :eval never :comments no :padline no :mkdirp yes
:END:

Deployed file: [[file:../lisp/uwumacs-languages.el][lisp/uwumacs-languages.el]]. Edit the blocks here; generated Lisp is kept in Git for ordinary startup and review.

** Group customization options

This Custom group gives the following settings a discoverable home in Emacs's built-in customization interface.

#+name: uwumacs-languages-01-1
#+begin_src emacs-lisp
...
Only request nix-mode when the user includes nix in uwumacs-language-packages. This does not install the Nix runtime or a language server.

#+name: uwumacs-languages-11-1
#+begin_src emacs-lisp
