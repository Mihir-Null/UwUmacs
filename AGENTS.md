...
The Nix layer (`flake.nix`, `nix/`) is checked with `nix flake check --no-build` and by evaluating `nix/example` with `--override-input uwumacs path:$PWD`; see `nix/README.org`.
