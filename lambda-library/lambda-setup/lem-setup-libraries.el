...
;; `async-start-process'.
(use-package async
  :when (and lem-load-extras (locate-library "async"))
  :after dired
  :config
  (dired-async-mode 1)
...
(use-package dash
  :when (and lem-load-extras (locate-library "dash"))
  :defer 2)
;; String manipulation
(use-package s
  :when (and lem-load-extras (locate-library "s"))
  :defer 2)
;; APIs for files & directories
(use-package f
  :when (and lem-load-extras (locate-library "f"))
  :defer 2)
