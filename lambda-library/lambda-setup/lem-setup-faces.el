...
(use-package outline-minor-faces
  :when (and lem-load-extras (locate-library "outline-minor-faces"))
  :after outline
...
(use-package dimmer
  :when (and lem-load-extras (locate-library "dimmer"))
...
(use-package svg-tag-mode
  :when (and lem-load-extras (locate-library "svg-tag-mode") (image-type-available-p 'svg))
...
(use-package highlight-numbers
  :when (and lem-load-extras (locate-library "highlight-numbers"))
  :defer t
...
(use-package hl-todo
  :when (and lem-load-extras (locate-library "hl-todo"))
  :defer t
...
;; TODO navigation transient. Bound to <leader> s t in
;; lem-setup-keybindings.el. Suffixes stay on press; "q" exits.
;;
;; Setup/teardown: the prefix's BODY runs at invocation, enables
;; `hl-todo-mode', and registers a one-shot `transient-exit-hook'
;; that disables it on every exit path (q, C-g, prefix re-invocation,
;; focus loss). The hook removes itself so subsequent exits do not
;; double-toggle. See git history for the prior macro-based dispatch
;; implementation this replaced.
(require 'transient)

(transient-define-prefix lem-todo-transient ()
  "TODO navigation."
  ["TODO"
   ("n" "Next"     hl-todo-next     :transient t)
   ("p" "Previous" hl-todo-previous :transient t)
   ("o" "Occur"    hl-todo-occur    :transient t)
  ...
  (transient-setup 'lem-todo-transient))
...
(use-package goggles
  :when (and lem-load-extras (locate-library "goggles"))
