...
;; A wrapping menu that layered two-letter mnemonics over
;; `org-agenda-mode-map' (`ds' schedule, `dd' deadline, `vw' week, etc.)
;; was deleted in Spec 09. Native agenda bindings cover the most common
;; operations. If any of the deleted mnemonics prove load-bearing,
;; restore them as direct `org-agenda-mode-map' bindings (Spec 09 O7).
;; See git history for the prior implementation.

;;;; Org Contrib
(use-package org-contrib
  :when (and lem-load-extras (locate-library "org-contrib"))
  :after org
  :config
...
;;;; Org Archive
;; Tell org where to archive completed tasks. Wrapped in
;; with-eval-after-load because `org-directory' is defined by org
;; and is not bound at require time. Before Spec 02 this ran at
;; top-level and crashed module-load with `void-variable
;; org-directory' whenever the file was required before org was
;; loaded (e.g. a cold sandbox boot with lem-load-extras nil).
(with-eval-after-load 'org
...
(defun lem-org-export-to-buffer-html-as-body (&optional async body-only ext-plist)
  "Export org buffer body to html"
  (interactive)
  (org-export-to-buffer 'html "*Org HTML Export*"
    async nil nil body-only ext-plist
    (lambda ()
      (html-mode)
