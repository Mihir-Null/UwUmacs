...
(use-package page-break-lines
  :when (and lem-load-extras (locate-library "page-break-lines"))
  :defer t
  :config
...
(use-package dashboard
  ;; Collapsed from two separate :if keys (use-package silently
  ;; dropped the first). Adds lem-load-extras gate and
  ;; locate-library check on nerd-icons, which :custom
  ;; references via (nerd-icons-sucicon ...).
  :when (and lem-load-extras
  ...
  ;; Header
  (dashboard-startup-banner (concat lem-library-dir
                                    (if (eq (car custom-enabled-themes) 'lambda-dark)
                                        "lambda-logo-white.png"
                                      "lambda-logo-black.png")))
  ...
  ;; Tell dashboard to use the nerd-icons backend for heading/file icons.
