...
Get it from:  <http://hasseg.org/trash/>"
    (let ((exit-code (call-process "trash" nil nil nil path)))
      (unless (eq exit-code 0)
        (error "Failed to trash %s (exit code %s)" path exit-code))))
...
(use-package reveal-in-osx-finder
  :when (and lem-load-extras (locate-library "reveal-in-osx-finder")) :defer 2)

;;;; Get mac links from safari
(use-package grab-mac-link
  :when (and lem-load-extras (locate-library "grab-mac-link")) :defer 1)
...
(use-package osx-lib
  :when (and lem-load-extras (locate-library "osx-lib")) :defer t)
