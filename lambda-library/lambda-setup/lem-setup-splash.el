...
;;; Code:
(require 'cl-lib)
;; Guarded require: without this, a sandbox boot with lem-load-extras
;; nil and no nerd-icons installed would crash module-load here.
;; Spec 02 caught the identical pattern in lem-setup-dashboard.el.
  ...
  (or (when (get-buffer "*splash*")
      ...
      (let* ((splash-buffer (get-buffer-create "*splash*")))
        ...
        (let* ((buffer-read-only t)
               ...
               ;; scale icons
               (nerd-icons-scale-factor 1.10)
               (nerd-icons-default-adjust -0.02))
          ...
          (with-current-buffer splash-buffer
            ...
            (let* ((center-col (max 0 (/ (- (window-width) 60) 2))))
              ...
              (when (and (> (window-width) 80) (> (window-height) 20))
                (let* ((image-file (if (eq (car custom-enabled-themes) 'lambda-dark)
            ...
            (let* ((button-indent (max 0 (/ (- (window-width) (length "Welcome to λ-Emacs")) 2)))
                   (button-labels '(("nf-fa-calendar" "Agenda" "a" lem-open-agenda-in-workspace "Open Agenda")
                                    ("nf-fa-code" "Config" "c" lem-open-emacsd-in-workspace "Visit config directory")
                                    ("nf-fa-envelope_o" "Mail" "m" lem-open-email-in-workspace "Open Email in Mu4e")
                                    ("nf-fa-book" "Notes" "n" lem-open-notes-in-workspace "Open notes directory")
                                    ("nf-fa-folder" "Projects" "p" tabspaces-open-or-create-project-and-workspace "Open project & workspace")))
              ...
              (dolist (button button-labels)
                (let* ((icon (nth 0 button))
                  ...
                  (insert-text-button (concat (nerd-icons-faicon icon)
                                              isep 
                                              padded-label 
                                              ksep 
                                              "(" key ")")
                                      'action (lambda (_) (lem-splash--workspace-dispatch action))
                                      'help-echo help
...
;;;; Workspace Dispatch
(defun lem-splash--workspace-dispatch (fn)
  "Call FN when it is defined, otherwise prompt to enable the workspaces module."
  (if (fboundp fn)
      (funcall fn)
    (message "Enable the workspaces module (lem-setup-workspaces) for this action")))

(defun lem-splash-open-agenda ()
  "Open the agenda workspace from the splash screen."
  (interactive)
  (lem-splash--workspace-dispatch 'lem-open-agenda-in-workspace))

(defun lem-splash-open-config ()
  "Open the emacs.d workspace from the splash screen."
  (interactive)
  (lem-splash--workspace-dispatch 'lem-open-emacsd-in-workspace))

(defun lem-splash-open-email ()
  "Open the email workspace from the splash screen."
  (interactive)
  (lem-splash--workspace-dispatch 'lem-open-email-in-workspace))

(defun lem-splash-open-notes ()
  "Open the notes workspace from the splash screen."
  (interactive)
  (lem-splash--workspace-dispatch 'lem-open-notes-in-workspace))

(defun lem-splash-open-project ()
  "Open a project workspace from the splash screen."
  (interactive)
  (lem-splash--workspace-dispatch 'lem-open-existing-project-and-workspace))

;;;; Define Minor Mode
;; Custom splash screen
(defvar lem-splash-mode-map
  (let ((map (make-sparse-keymap)))
    (define-key map (kbd "a") 'lem-splash-open-agenda)
    (define-key map (kbd "c") 'lem-splash-open-config)
    (define-key map (kbd "m") 'lem-splash-open-email)
    (define-key map (kbd "n") 'lem-splash-open-notes)
    (define-key map (kbd "p") 'lem-splash-open-project)
    (define-key map (kbd "q") 'lem-splash-screen-kill)
    (define-key map (kbd "<escape>") 'lem-splash-screen-bury)
...
(define-minor-mode lem-splash-mode
  ...
  ;; Use Emacs state if Evil is active so single-key bindings work
