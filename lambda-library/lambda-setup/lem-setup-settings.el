...
;; Emacs-Dots: saved preferences are persistent state, not disposable cache.
(use-package cus-edit
  :ensure nil
  :defer 1
  :custom
  (custom-file (expand-file-name "custom.el" lem-etc-dir))
  :config
...
;;;;; Private File
;; Emacs-Dots loads private.el once from config.el after platform defaults exist.
...
(use-package savehist
  ...
  :config
  (setq savehist-file (concat lem-cache-dir "savehist"))
  (unless (file-exists-p savehist-file)
    (make-empty-file savehist-file t))
...
(use-package desktop
  ...
  (setq desktop-dirname             (concat lem-cache-dir "desktops")
        ...
        desktop-files-not-to-save   "\\(\\`/[^/:]*:\\|(ftp)\\'\\)\\|\\`\\'\\|magit"
...
(use-package expand-region
  :when (and lem-load-extras (locate-library "expand-region"))
  :defer 1)
