...
;;;; Icomplete Vertical (built-in baseline)
;; EMACS-28+ built-in. Baseline minibuffer completion UI used when
;; lem-load-extras is nil (i.e. when vertico is absent). When
;; lem-load-extras is t, the vertico form below explicitly disables
;; icomplete in its :init so both UIs never run simultaneously.
;; C-2 load order: this block sits at the top of the file so it
;; fires at load time BEFORE use-package expands the vertico form
;; below; vertico's :init then cancels it in the same file load.
;; Do not move this block below the vertico form or the sequence
;; breaks silently.
...
;; (icomplete-mode 1) above force-loads the `icomplete' feature,
;; so `icomplete-minibuffer-map' is bound by the time these
;; define-key calls fire. No with-eval-after-load wrapper needed.
(define-key icomplete-minibuffer-map (kbd "C-v")   #'icomplete-vertical-toggle)
(define-key icomplete-minibuffer-map (kbd "RET")   #'icomplete-force-complete-and-exit)
(define-key icomplete-minibuffer-map (kbd "TAB")   #'icomplete-force-complete-and-exit)
(define-key icomplete-minibuffer-map (kbd "C-M-i") #'minibuffer-complete)
...
;;;; Completion Preview (built-in baseline)
;; EMACS-30+ built-in (autoloaded via loaddefs). Baseline in-buffer
;; completion preview on prog-mode buffers. Raw top-level add-hook;
;; BR-3 explicitly forbids wrapping this in with-eval-after-load
;; 'completion-preview -- that would fail to fire in batch because
;; the feature is never force-loaded, silently breaking AC-4.
...
(use-package vertico
  :when (and lem-load-extras (locate-library "vertico"))
  :bind (:map vertico-map
         ("<escape>" . #'minibuffer-keyboard-quit)
         ("M-RET"    . #'vertico-exit))
  :hook (emacs-startup . vertico-mode)
  :init
  ;; C-2: cancel the inline icomplete baseline at use-package
  ;; expansion time (load-time, before emacs-startup-hook fires
  ;; vertico-mode). These two calls MUST remain bare top-level
  ;; s-expressions in :init -- AC-2 asserts their textual presence
  ;; via the Spec 02 classifier. Do not wrap in when/unless/etc.
  (icomplete-vertical-mode -1)
  (icomplete-mode -1)
  :config
...
(use-package orderless
  :when (and lem-load-extras (locate-library "orderless"))
  :init
...
(use-package embark
  :when (and lem-load-extras (locate-library "embark"))
  ...
  ;; Which-key integration. Built-in which-key only (Emacs 29+);
  ;; BR-6 deleted the dead external which-key--* fallback branches.
  (defun embark-which-key-indicator ()
    "An embark indicator that displays keymaps using built-in which-key."
...
(use-package marginalia
  :when (and lem-load-extras (locate-library "marginalia"))
...
(use-package consult
  :when (and lem-load-extras (locate-library "consult"))
...
(use-package consult-dir
  :when (and lem-load-extras (locate-library "consult-dir"))
...
;;;;; Corfu
(use-package corfu
  :when (and lem-load-extras (locate-library "corfu"))
  :hook
...
;; Add extensions
(use-package cape
  :when (and lem-load-extras (locate-library "cape"))
...
(use-package kind-icon
  :when (and lem-load-extras (locate-library "kind-icon"))
  :defer 1
  :custom
...
(use-package yasnippet
  :when (and lem-load-extras (locate-library "yasnippet"))
  :defer 1
  :bind (:map yas-minor-mode-map
         ("C-'" . yas-expand))
  :init
  ;; Create snippet directories at load time. Was `:preface' which
  ;; also ran at byte-compile time, triggering a void-variable on
  ;; `lem-all-snippets-dir' because the preceding top-level
  ;; defcustom is not eval'd by `byte-compile-file'. `:init' runs
  ;; at load time only, after the defcustom has bound the variable.
