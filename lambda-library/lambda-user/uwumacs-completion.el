;;; uwumacs-completion.el --- Minibuffer and in-buffer completion -*- lexical-binding: t; -*-
...
(use-package vertico
  ...
  (vertico-mode 1)
  :config
  (defun uwumacs-vertico-directories-first (files)
  ...
  (setopt vertico-multiform-categories '((file (vertico-sort-function . uwumacs-vertico-directories-first))))
...
(defun uwumacs--crm-indicator (args)
  "Prefix a `completing-read-multiple' prompt with [CRM]."
  (cons (concat "[CRM] " (car args)) (cdr args)))
(advice-add #'completing-read-multiple :filter-args #'uwumacs--crm-indicator)
...
(defun uwumacs-search-symbol-at-point ()
  "Search this buffer's lines for the symbol at point."
  (interactive)
  (consult-line (thing-at-point 'symbol)))

(defun uwumacs-search-manuals ()
...
(use-package embark
  ...
  :config
  (defun uwumacs-embark-which-key-indicator ()
        ...
        (which-key-show-keymap
         (if (eq (plist-get (car targets) :type) 'embark-become)
           ...
           (format "Act on %s '%s'%s"
                   ...
                   (if (cdr targets) "..." "")))))))
  (setopt embark-indicators '(uwumacs-embark-which-key-indicator
                              embark-highlight-indicator
                              embark-isearch-highlight-indicator))
  (defun uwumacs--embark-hide-which-key (function &rest args)
    "Hide the which-key popup before the completing-read prompter takes over."
    (which-key-hide-popup)
    (let ((embark-indicators (remq #'uwumacs-embark-which-key-indicator embark-indicators)))
      (apply function args)))
  (advice-add #'embark-completing-read-prompter :around #'uwumacs--embark-hide-which-key)
  (add-to-list 'display-buffer-alist
               '("\\`\\*Embark Collect \\(Live\\|Completions\\)\\*"
                 nil
                 (window-parameters (mode-line-format . none))))
  (defun uwumacs-embark-dired-here (file)
    "Open Dired in FILE's directory."
    (dired (file-name-directory file)))
  (defun uwumacs-embark-ripgrep-here (file)
    "Run ripgrep in FILE's directory."
    (let ((default-directory (file-name-directory file)))
      (consult-ripgrep)))
  (keymap-set embark-file-map "x" #'embark-open-externally)
  (keymap-set embark-file-map "D" #'uwumacs-embark-dired-here)
  (keymap-set embark-file-map "g" #'uwumacs-embark-ripgrep-here)
...
(use-package corfu
  ...
  (defun uwumacs--corfu-in-minibuffer ()
    "Enable Corfu in minibuffers that offer completion at point, such as M-:."
    (when (where-is-internal #'completion-at-point (list (current-local-map)))
      (corfu-mode 1)))
  (add-hook 'minibuffer-setup-hook #'uwumacs--corfu-in-minibuffer)
  (defun uwumacs--corfu-in-shells ()
    "In shells, complete only on request and accept without a second RET."
    (setq-local corfu-auto nil
                corfu-quit-no-match t
                corfu-quit-at-boundary t)
    (corfu-mode 1))
  (add-hook 'eshell-mode-hook #'uwumacs--corfu-in-shells)
  (defun uwumacs--corfu-send-shell (&rest _)
    ...
    (cond ((and (derived-mode-p 'eshell-mode) (fboundp 'eshell-send-input))
          ...
          ((and (derived-mode-p 'comint-mode) (fboundp 'comint-send-input))
           (comint-send-input))))
  (advice-add #'corfu-insert :after #'uwumacs--corfu-send-shell))
...
(defvar-keymap uwumacs-cape-map
...
(use-package yasnippet
  ...
  :bind (:map yas-minor-mode-map
         ("C-'" . yas-expand))
  :custom
  (yas-snippet-dirs (list uwumacs-snippets-dir))
  :config
  (make-directory uwumacs-snippets-dir t)
  (defun uwumacs--yas-not-in-org-src ()
    "Do not expand snippets inside Org source blocks."
    (setq-local yas-buffer-local-condition '(not (org-in-src-block-p t))))
  (add-hook 'org-mode-hook #'uwumacs--yas-not-in-org-src)
  (with-eval-after-load 'warnings
    (push '(yasnippet backquote-change) warning-suppress-types))
  (yas-global-mode 1))

(provide 'uwumacs-completion)
;;; uwumacs-completion.el ends here
